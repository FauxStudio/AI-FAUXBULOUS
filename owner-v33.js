/* AI-FAUXBULOUS Rebecca Owner Mode v35 */
(()=>{'use strict';
...
`;
function addStyle(){if($('#ownerV35Style'))return;const s=document.createElement('style');s.id='ownerV35Style';s.textContent=css;document.head.appendChild(s)}
...
function addOwnerBar(){if(!ownerView()||$('#rebeccaOwnerBar'))return;const h=host();if(!h)return;const bar=document.createElement('div');bar.id='rebeccaOwnerBar';bar.innerHTML='<div><b>👑 REBECCA OWNER MODE</b><small>Your private creator controls are active.</small></div><button type="button">USER VIEW</button>';bar.querySelector('button').onclick=()=>{localStorage.setItem(VIEW_KEY,'0');apply()};h.before(bar)}
...
function boot(){addStyle();apply();setTimeout(apply,250);setTimeout(apply,900);new MutationObserver(()=>{clearTimeout(window.__ownerV35T);window.__ownerV35T=setTimeout(()=>{apply();revealSuite()},60)}).observe(document.body,{childList:true,subtree:true})}
